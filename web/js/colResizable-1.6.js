...
(function ($) {
    ...
    var init = function (tb, options) {
        ...
        // t.w = t.width();
        t.w = initTableWidth;
        t.head.width(t.w).removeAttr('width');
        $("#book-table-header").width(t.w).removeAttr('width');
        t.width(t.w);
        ...
        t.cs = I(ie ? tb.cellSpacing || tb.currentStyle.borderSpacing : t.css('border-spacing'));	//table cellspacing (not even jQuery is fully cross-browser)
        t.b = I(ie ? tb.border || tb.currentStyle.borderLeftWidth : t.css('border-left-width'));	//outer border width (again cross-browser issues)
    ...
    var initCol = function (t) {
        ...
        th.each(function (i) {						//iterate through the table column headers
            ...
            t.head.th.push($("#header th").eq(i));
            // t.head.th.eq[i];
            g.t = t;
            g.i = i;
            g.c = c;
            c.w = initC[i];		//some values are stored in the grip's node data as shortcut
            t.head.th[i].width(c.w).removeAttr('width');
            // t.head.th[i].removeAttr('width');
            t.g.push(g);
    ...
    var memento = function (t, th) {
        ...
        if (th) {										//in deserialization mode (after a postback)
            ...
            for (; i < t.ln; i++) {						//for each column
                ...
                aux.push(w[i]); 	//width is stored in an array since it will be required again a couple of lines ahead
    ...
    var syncCols = function (t, i, isOver) {
        ...
        var w = M.max(c.w + inc, 0);
        // var w2 = c2.w - inc;	//their new width is obtained
        c.width(w + PX);
        // t.head.th[i].width(w+PX);
    ...
    var applyBounds = function (t) {
        ...
        for (i = 0; i < t.c.length; i++) {
            // if (t.c[i].w <= t.c[i].width()) {
            if (t.c[i].w <= 0) {
                t.c[i].w = t.c[i].width();
                t.head.th[i].w = t.c[i].width();
            }
            x += t.c[i].w;
        }
        // t.w = x;
    ...
    var onGripDrag = function (e) {
        ...
        // var max = Infinity; 								//max position according to the contiguous cells
        x = M.max(x, 0);				//apply bounding
        drag.x = x;
...
//                html.text('<span class="glyphicon glyphicon-ok"></span>');
    ...
    //             for (i = 0; i < t.ln; i++) {
    //                 sum += t.c[i].w;
    //             //     t.c[i].css("width", t.c[i].w).l = true;
    //             //     if (i == last) {
    //             //         if (sum < t.width()) {
    //             //             t.c[i].css("width", t.width() - j - sum).l = true;
    //             //         } else {
    //             //             // t.c[i].
    //             //             console.log('last');
    //             //         }
    ...
    //             //
    //             // if (sum < t.width()) {
    //             //     console.log(sum, t.width());
    //             // }
    //             // console.log(t.width() - sum);
    //             t.w = sum;
    //             console.log(t.w)
    ...
    //                 for(i=0; i<t.ln; i++) t.c[i].css("width", t.c[i].w).l=true;
